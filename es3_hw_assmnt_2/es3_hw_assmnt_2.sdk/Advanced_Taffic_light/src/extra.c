...
void excute(u16 button, XGpio P_BTN, int direction)
{
	...
	switch(direction)
	{
		case 1:	//UP
			...
			//it excutes once in one interrupt, so the speed of the car is 0.7 pixel/0.004s
			break;
		case 2:	//LEFT
			if(button)
				target_x=accelerate(target_x, 1 SECOND, 0, -1, &Car_speed);
			else
				target_x=accelerate(target_x, 1 SECOND, -1, 0, &Car_speed);
				//target_x--;
			break;
		case 3:	//DOWN
			if(button)
