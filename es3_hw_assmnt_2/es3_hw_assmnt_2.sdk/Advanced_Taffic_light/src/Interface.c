...
The getSwitch() function is to get the value of a specific bit of the slide switch.
...
/*This function is to get the value of a specific bit of the slide switch.
 * Please note, the parameter "position" is 1,2,3,...,16.
 */
int getSwitch(int position)
{
	...
	for(i=0;i<=15;i++)
	{
		SwichValue[i] = SwitchIn%2;	//to store each bit into the array
		SwitchIn = SwitchIn/2;
	}

	SwitchOut = SwichValue[position-1];	//output the result
	return SwitchOut;
}
